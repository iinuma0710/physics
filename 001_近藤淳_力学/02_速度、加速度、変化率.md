...
座標の原点から質点に向かって引いたベクトルを質点の**位置ベクトル** $\boldsymbol{r}$ と呼び、$\boldsymbol{r} = (x, y, z)$ と表します。
...
一方、質点が位置ベクトル $\boldsymbol{r}$ の点 P から $\boldsymbol{r}'$ の点 P' に変位したとき、点 P から点 P' に引いたベクトルは $\boldsymbol{r}' - \boldsymbol{r}$ で表され、原点の取り方に依存しません。
...
質点の位置ベクトル $\boldsymbol{r}$ は $t$ の関数であり、$\boldsymbol{r} = \boldsymbol{r}(t) = (x(t), y(t), z(t))$ と表します。
1次元の場合と同様に時刻 $t$ から $t + \Delta t$ の変位を考えると、その変位ベクトル $\Delta \boldsymbol{r}$ は以下の式で表されます。

$$
\Delta \boldsymbol{r} = \boldsymbol{r}(t + \Delta t) - \boldsymbol{r}(t)
$$

そして、平均速度ベクトル $\Delta \boldsymbol{r} / \Delta t$ の $\Delta t$ に関するゼロ極限をとると、時刻 $t$ における速度ベクトル $\boldsymbol{\dot{r}}=\boldsymbol{v}(t)$ が得られます。

$$
\boldsymbol{v}(t) = \lim_{\Delta t \rightarrow 0} \frac{\Delta \boldsymbol{r}}{\Delta t} = \frac{d \boldsymbol{r}}{dt} = \boldsymbol{\dot{r}} = (\dot{x}, \dot{y}, \dot{z})
$$
...
$$
\boldsymbol{a}(t) = \dot{\boldsymbol{v}}(t) = \ddot{\boldsymbol{x}}(t)
$$
...
この加速度ベクトルは、上の図より $- \boldsymbol{r}$ の方向を向いています。
...
速度ベクトルを $\boldsymbol{v}(t)$、$t=t_0$ の時の位置ベクトルを $\boldsymbol{r}_0$ とすると、位置ベクトル $\boldsymbol{r}(t)$ は以下の式で与えられます。

$$
\boldsymbol{r}(t) - \boldsymbol{r}_0 = \int_{t_0}^t \boldsymbol{v}(t) dt
$$